...
// github.com/datadope-io/go-zabbix.git  v2 version
// add connection pool
package sender
...
func NewSender(addr string, connectTimeout, readTimeout, writeTimeout time.Duration, poolSize int) *Sender {
	if poolSize <= 0 {
...
func (s *Sender) Send(packet *Packet) (res Response, err error) {
	...
	for retry = 0; retry < s.MaxRetries; retry++ {
		// 从池中获取或新建连接
		conn, err = s.Pool.Get(s.ServerAddr, s.ConnectTimeout)
		if err != nil {
			fmt.Printf("Get connection from pool error: %s, will retry", err)
			continue // 重试
		}
	...
	// 重试次数用尽
	return res, fmt.Errorf("max retries (%d) exceeded: %w", s.MaxRetries, err)
}
